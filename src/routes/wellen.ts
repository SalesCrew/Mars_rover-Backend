...
// ============================================================================
// SHARED HELPER: Resolve einzelprodukt names from both wave-local and master tables
// Needed because GLs can submit items from the master `products` catalogue
// under item_type='einzelprodukt'. Wave-local wellen_einzelprodukte entries take
// precedence; products table is the fallback (including soft-deleted rows so
// historical submissions keep their names after archiving).
// ============================================================================
  ...
  try {
    ...
    const chainAverages = await Promise.all([
      ...
      (async () => {
        ...
        // Get einzelprodukte for value calculation
        const { data: einzelprodukteZoo } = await freshClient
          .from('wellen_einzelprodukte')
          .select('id, item_value, welle_id')
          .in('welle_id', welleIds);

        // Get progress (paginated, including einzelprodukt)
        let displayProgress: any[] = [];
        let kartonwareProgress: any[] = [];
        let paletteProgressZoo: any[] = [];
        let schutteProgressZoo: any[] = [];
        let einzelproduktProgressZoo: any[] = [];
        
        {
          let allChainSubs: any[] = [];
          let chainSubFrom = 0;
          const chainPageSize = 1000;
          let chainHasMore = true;
          while (chainHasMore) {
            let query = freshClient.from('wellen_submissions')
              .select('welle_id, gebietsleiter_id, item_type, item_id, quantity, value_per_unit')
              .in('welle_id', welleIds);
            if (glFilter.length > 0) query = query.in('gebietsleiter_id', glFilter);
            const { data } = await query.range(chainSubFrom, chainSubFrom + chainPageSize - 1);
            if (data && data.length > 0) {
              allChainSubs = [...allChainSubs, ...data];
              chainSubFrom += chainPageSize;
              chainHasMore = data.length === chainPageSize;
            } else {
              chainHasMore = false;
            }
          }
          const aggregated = aggregateSubmissions(allChainSubs);
          if (!itemType || itemType === 'displays') displayProgress = aggregated.filter(p => p.item_type === 'display');
          if (!itemType || itemType === 'kartonware') kartonwareProgress = aggregated.filter(p => p.item_type === 'kartonware');
          paletteProgressZoo = aggregated.filter(p => p.item_type === 'palette');
          schutteProgressZoo = aggregated.filter(p => p.item_type === 'schuette');
          einzelproduktProgressZoo = aggregated.filter(p => p.item_type === 'einzelprodukt');
        }
        ...
        // Calculate current value using dashboard-identical logic
        let currentValue = 0;
        ...
        for (const progress of einzelproduktProgressZoo) {
          const ep = (einzelprodukteZoo || []).find((e: any) => e.id === progress.item_id);
          if (ep) currentValue += progress.current_number * (ep.item_value || 0);
        }
      ...
      (async () => {
        ...
        // Get einzelprodukte for value calculation
        const { data: einzelprodukteHage } = await freshClient
          .from('wellen_einzelprodukte')
          .select('id, item_value, welle_id')
          .in('welle_id', welleIds);
        ...
        
        {
          ...
          einzelproduktProgress = aggregated.filter(p => p.item_type === 'einzelprodukt');
        }
        ...
        for (const progress of einzelproduktProgress) {
          const ep = (einzelprodukteHage || []).find((e: any) => e.id === progress.item_id);
          if (ep) currentValue += progress.current_number * (ep.item_value || 0);
        }
  ...
  try {
    ...
    const wavesProgress = await Promise.all(
      (wellen || []).map(async (welle) => {
        ...
        // Calculate current value for value-based goals using wellen_submissions (source of truth)
        let currentValue = 0;
        if (welle.goal_type === 'value') {
          ...
          for (const sub of allSubmissionsData) {
            if (sub.item_type === 'display') {
              ...
              if (display) {
                currentValue += sub.quantity * (display.item_value || 0);
              }
            } else if (sub.item_type === 'kartonware') {
              const kw = kartonware.find(k => k.id === sub.item_id);
              if (kw) {
                currentValue += sub.quantity * (kw.item_value || 0);
              }
            } else if (sub.item_type === 'palette' || sub.item_type === 'schuette') {
              currentValue += sub.quantity * (sub.value_per_unit || 0);
            } else if (sub.item_type === 'einzelprodukt') {
              const ep = einzelprodukte.find(e => e.id === sub.item_id);
              // For wave-local items use item_value; for master products fall back to submitted value_per_unit
              currentValue += sub.quantity * (ep?.item_value || sub.value_per_unit || 0);
            }
          }
        }
...
async function fetchWellenPhotoRowsForAdmin(
  ...
  return (data || []).map((photo: any) => {
    ...
    return {
      ...
      welleName: welleMap.get(photo.welle_id) || '',
      fragebogenId: '',
      fragebogenName: '',
...
router.get('/:welleId/gl-submissions/:glId', async (req: Request, res: Response) => {
  try {
    ...
    const standardResponses = standardEntries.map(entry => {
      ...
      else item = einzelprodukteMap.get(entry.item_id);
    ...
    const paletteEntries = submissions.filter(s => s.item_type === 'palette');
    ...
    for (const entry of paletteEntries) {
      ...
      const key = `${entry.market_id}|${parentId}|${timestampKey}`;
    ...
    for (const [, entries] of paletteGroups) {
      ...
      paletteResponses.push({
        ...
        quantity: 1,
        valuePerUnit: 0,
        value: products.reduce((sum: number, p: any) => sum + p.value, 0),
        timestamp: firstEntry.created_at,
        products
      });
    }

    // Group schuette entries
    const schutteEntries = submissions.filter(s => s.item_type === 'schuette');
    ...
    for (const entry of schutteEntries) {
      ...
      const timestampKey = new Date(entry.created_at).toISOString().slice(0, 16);
      const key = `${entry.market_id}|${parentId}|${timestampKey}`;
      if (!schutteGroups.has(key)) schutteGroups.set(key, []);
  ...
  try {
    ...
    const standardResponses = displayKartonwareEinzelproduktEntries.map(entry => {
      ...
      if (entry.item_type === 'display') {
        ...
        item = einzelprodukteMap.get(entry.item_id);
      }
        ...
        for (const [, subs] of subsByTimestamp) {
          ...
          for (const s of displayKartonwareEinzelproduktSubs) {
            ...
            if (s.item_type === 'display') {
              ...
              const e = einzelprodukteMap.get(s.item_id);
...
router.get('/export/submissions', async (req: Request, res: Response) => {
  try {
    ...
    // Build item name map — einzelprodukt uses dual-source resolver
...
router.get('/market/:marketId/pending-photos', async (req: Request, res: Response) => {
  try {
    ...
    const [displays, kartonware, paletteProducts, schutteProducts] = await Promise.all([
      displayIds.length > 0 
